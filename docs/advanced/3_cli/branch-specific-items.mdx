...
- Windmill workspace: `f/env_staging/folder.meta.yaml`
- Local: `f/env_staging/folder.main.meta.yaml` (on main branch)
- Windmill workspace: `settings.yaml`
- Local: `settings.main.yaml` (on main branch)
...
- Windmill workspace: `u/alex/orders.kafka_trigger.yaml`
- Local: `f/env_staging/folder.dev.meta.yaml` (on dev branch)
- Windmill workspace: `f/env_staging/folder.meta.yaml`
- Local: `settings.dev.yaml` (on dev branch)
- Windmill workspace: `settings.yaml`
...
You can override the current Git branch using the `--branch` flag. This is useful when:
- Working outside a Git repository
- Testing branch-specific configurations without switching branches
- Running sync operations in CI/CD environments where the Git branch may not be detected correctly

```bash
# Pull using main branch configuration, regardless of current Git branch
wmill sync pull --branch main

# Push using staging branch settings
wmill sync push --branch staging
```

When using `--branch`, the CLI:
- Uses the specified branch's configuration from `wmill.yaml`
- Applies branch-specific item transformations for that branch
- Skips branch validation prompts automatically
...
3. **Pattern testing**: Use tools like [https://globster.xyz](https://globster.xyz) to test your patterns
